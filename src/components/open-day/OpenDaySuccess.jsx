...
// Dedicated success URL for an Open Day Elite Trial registration (per-centre,
// e.g. /PGP2026/williamstown/success). The registration form inserts the row,
// stashes the player's first name + parent email in sessionStorage, then
// navigates here. This gives a unique thank-you URL where the Meta Pixel
// conversion fires — attributable/optimisable in Meta Ads and usable for a
// URL-based Custom Conversion. The base Pixel (fbq init + PageView) lives
// globally in index.html; here we register a PageView for this SPA route change
// plus the Lead conversion, firing ONCE per real registration.
...
const fireMetaConversion = (config) => {
    try {
        if (typeof window !== 'undefined' && typeof window.fbq === 'function') {
            ...
            window.fbq('track', 'Lead', {
                content_name: config.pixelName,
                content_category: config.pixelCategory,
            });
        }
        ...
        <section className="relative overflow-hidden bg-rr-dark text-white min-h-screen flex items-center justify-center px-6 py-16 selection:bg-rr-pink selection:text-white">
            ...
            <div className="relative z-10 max-w-2xl w-full mx-auto text-center">
                ...
                <InfoPanel delay={0.55} title="Your Session" icon="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z">
                    <p className="text-sm text-white font-bold mb-1">{config.successSessionLine}</p>
                    <p className="text-sm text-white/60 leading-relaxed">{config.successVenueLine}</p>
                </InfoPanel>
